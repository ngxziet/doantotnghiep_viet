...
static constexpr int CONFIG_VERSION = 5;  // v5: ENCODER_PULSES_PER_REV 20→40 (LM393 doubling ở mọi tốc độ) → reset encoderScale
...
// SỐ XUNG MCU ĐẾM ĐƯỢC mỗi vòng bánh — là MẪU SỐ quy đổi xung→cm (CM_PER_PULSE = chu_vi / hằng_số).
// Đĩa vật lý 20 KHE nhưng comparator LM393 thiếu hysteresis → NHÂN ĐÔI cạnh → MCU đếm ~40 xung/vòng.
// Test thực tế (encoder_calib_test, kê xe lên, motor tự quay): firmware đếm 20 xung thì bánh chỉ quay
// ~½ vòng → xác nhận 40 xung/vòng kể cả khi motor quay (không chỉ lúc quay tay chậm).
// → lấy 40 cho khớp số MCU đếm thật. (Fix triệt để doubling: vặn biến trở module / thêm Schmitt 74HC14 → về 20.)
...
// 50cm / 0.51 ≈ 98 xung. Fine-tune: chạy thật 1 node, đo X cm → PULSES_PER_NODE = 98 × 50 / X.
...
static constexpr int SERVO_MIN_PULSE_US = 500;  // xung ứng với 0° — SG90 cần 0.5ms cho góc tối thiểu (đủ ±90°)
static constexpr int SERVO_MAX_PULSE_US = 2500; // xung ứng với 180° — SG90 cần 2.5ms cho góc tối đa (1-2ms chỉ ±45°)
